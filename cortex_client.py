...
class CortexClient:
    ...
    def generate_sql_and_results(self, enhanced_query: str) -> Dict[str, Any]:
        ...
        with self._get_session() as session:
            ...
            with Timer("schema_fetch"):
                table_schema = self._get_table_schema(session, target_db, target_schema, "IOT_TELEMETRY_DATA")
            
            log_event("schema_fetched", database=target_db, schema=target_schema)
            
            prompt = self._build_prompt(enhanced_query, session)
            log_event("cortex_prompt_built", prompt_length=len(prompt))
            
            cortex_sql = self._build_cortex_statement(prompt)
            cleaned_sql = ""

            try:
                with Timer("cortex_call"):
                    generated_sql = self._invoke_cortex(session, cortex_sql)
                
                cleaned_sql = self._clean_sql(generated_sql)
                log_event("sql_generated", sql_length=len(cleaned_sql))
                
                with Timer("sql_execute"):
                    rows, columns = self._execute_sql(session, cleaned_sql)
                
                log_event("sql_executed", row_count=len(rows), column_count=len(columns))
                
            except SnowparkSQLException as exc:
                error_detail = str(exc)
                error_type = type(exc).__name__
                log_event("sql_error", level="ERROR", error=str(exc), generated_sql=cleaned_sql, error_type=error_type)
                ...
                error_type = type(exc).__name__
                log_event("unexpected_error", level="ERROR", error=str(exc), error_type=error_type)
    ...
    def _build_prompt(self, enhanced_query: str, session: Session) -> str:
        ...
        schema_context = (
            ...
            f"6. For numeric comparisons (e.g., 'less than 70', 'more than 50'):\n"
            f"   - Use the numeric column from the schema\n"
            f"   - Compare with the numeric value ONLY (e.g., WHERE column_name < 70)\n"
            f"   - DO NOT use date strings in numeric comparisons\n"
            f"7. For date comparisons: Only use date/timestamp columns from schema, never use '{current_date}' in WHERE clauses unless explicitly requested\n"
            f"8. IGNORE any date context in the user question unless the query explicitly asks for date filtering\n"
            f"9. Output ONLY the SQL query - no markdown code blocks, no explanations\n\n"
